# FUTURE_PE_01
# AI Website Copy Generator for Local Businesses

## Task
Future Interns – Task 1

## Project Title
Beauty Salon Website Copy

## Objective
To create professional, persuasive, and website-ready copy for a local beauty salon using AI-powered prompt engineering.

## AI Tool Used
- ChatGPT

## Business Type
Beauty Salon

## Deliverables
- Homepage
- About Us
- Services
- Why Choose Us
- Contact Us
- Call-to-Action (CTA)

## Files Included
- Beauty_Salon_Website_Copy.docx
- Prompts.txt
- README.md

## Skills Gained
- Prompt Engineering
- Website Copywriting
- AI Content Generation
- Business Communication
- Marketing Content Creation

## Outcome
Successfully created a complete AI-generated website copy that is clear, engaging, and ready for use on a business website.

## Author
Your Name
